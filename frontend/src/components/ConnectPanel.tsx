...
export function ConnectPanel() {
  const { status, loading, error, connect, disconnect, disconnecting } = useConnection();
  ...
  return (
    <div className={`bg-white rounded-2xl border-2 p-6 transition-all duration-300 ${
      connected ? 'border-emerald-200 shadow-sm shadow-emerald-100' : 'border-slate-200'
    }`}>
      <div className="flex items-start justify-between">
        <div>
          ...
          {/* <h2 className="text-xl font-bold text-slate-900 mb-1">HubSpot Account</h2> */}
          {connected && status?.portalId && (
            <p className="text-sm text-slate-500">
              Portal ID: <span className="font-mono text-slate-700">{status.portalId}</span>
            </p>
          )}
          {!connected && (
            <p className="text-sm text-slate-500 mt-1">
              Connect your HubSpot account to enable contact syncing and form capture.
            </p>
          )}
        </div>
        ...
        <div className="flex items-center gap-2 bg-slate-50 rounded-xl px-3 py-2">
          <span className="text-[#0C6EFC]"><WixIcon /></span>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-slate-400">
            <path d="M17 7l-10 10M7 7l10 10" />
          </svg>
          <span className="text-[#ff7a59]"><HubSpotIcon /></span>
        </div>
      </div>
